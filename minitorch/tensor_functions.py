...
class Log(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        (t1,) = ctx.saved_tensors
        reciprocal_t1 = t1.f.inv_map(t1)
        grad_input = t1.f.mul_zip(grad_output, reciprocal_t1)
        return grad_input
...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        grad_input = grad_output.f.mul_zip(grad_output, exp_result)
        return grad_input
...
class IsClose(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        """Compute gradients for the is-close comparison (no gradients)."""
        return zeros(grad_output.shape), zeros(grad_output.shape)
